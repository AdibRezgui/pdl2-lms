...
@Component({
    ...
    <div class="flex h-screen overflow-hidden" style="background:linear-gradient(160deg,#fffdfb 0%,#fdf2f8 60%,#f6f0ff 100%)">
      ...
      <main class="flex-1 overflow-y-auto p-7">
        <header class="mb-7 reveal">
          <h1 class="font-display text-2xl font-bold" style="color:#221f2c">Mon Parcours <span class="gt">Personnalisé</span></h1>
          <p class="text-sm mt-0.5" style="color:#948da3">Votre chemin d'apprentissage adapté par l'IA</p>
        </header>
        ...
        <div *ngIf="analysis()" class="level-card reveal stagger-1 mb-6">
          <div class="level-left">
            ...
            <div>
              <p class="text-xs font-semibold" style="color:#948da3; letter-spacing:.04em; text-transform:uppercase">Votre niveau actuel</p>
              <p class="font-display font-bold text-2xl" style="color:#221f2c">{{ analysis()!.overallLevel }}</p>
              <p class="text-xs mt-1" style="color:#948da3">Progression moyenne : {{ analysis()!.averageProgress }}%</p>
            </div>
          </div>
        ...
        <div class="two-col">
          ...
          <section class="reveal stagger-2">
            ...
            <div *ngIf="!loadingEnrollments() && activeEnrollments().length === 0" class="card p-8 text-center">
              ...
              <p class="font-semibold" style="color:#221f2c">Aucun cours en cours</p>
              ...
              <div *ngFor="let enr of activeEnrollments(); let i = index; let last = last" class="path-step">
                ...
                <div class="step-card card p-4">
                  <div class="flex items-start justify-between mb-2">
                    <div>
                      <p class="text-sm font-bold" style="color:#221f2c">{{ enr.course.title }}</p>
                      <p class="text-xs" style="color:#948da3">{{ enr.course.category }} · {{ levelLabel(enr.course.level) }}</p>
                    </div>
                  ...
                  <div class="flex items-center gap-3">
                    <div class="path-prog-track flex-1">
                      <div class="path-prog-fill" [style.width.%]="enr.progress"
                        [style.background]="enr.progress >= 80 ? 'linear-gradient(90deg,#6ee7b7,#1f9d6f)' : enr.progress >= 40 ? 'linear-gradient(90deg,#a78bfa,#fb7299)' : 'linear-gradient(90deg,#fda4af,#f25c78)'">
                      </div>
                    </div>
                    <span class="text-xs font-bold" style="color:#8b6ef2;min-width:32px">{{ enr.progress }}%</span>
          ...
          <section class="reveal stagger-3">
            ...
            <div class="space-y-3">
              <div *ngFor="let rec of recommendations(); let i = index" class="rec-card card p-4 lift-on-hover">
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-bold mb-0.5" style="color:#221f2c">{{ rec.title }}</p>
                    <p class="text-xs" style="color:#948da3">{{ rec.category }} · {{ levelLabel(rec.level) }}</p>
                    <p *ngIf="rec.reason" class="text-xs mt-1 italic" style="color:#a78bfa">{{ rec.reason }}</p>
                  </div>
              ...
              <div *ngIf="!loadingRecs() && recommendations().length === 0" class="card p-6 text-center">
                <p class="text-sm" style="color:#948da3">L'IA analyse votre profil pour vous proposer des cours</p>
              </div>
            </div>
            ...
            <div *ngIf="analysis() && analysis()!.recommendations.length > 0" class="mt-6">
              ...
              <div class="space-y-2">
                <div *ngFor="let tip of analysis()!.recommendations" class="tip-card">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" stroke-width="2" style="flex-shrink:0;margin-top:1px"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                  <p class="text-xs" style="color:#4a4458">{{ tip }}</p>
                </div>
              </div>
            </div>
          </section>
  ...
  styles: [`
    ...
    .section-title { font-family:'Fraunces',Georgia,serif; font-weight:700; font-size:16px; color:#221f2c; }
    .level-card { display:flex; align-items:flex-start; gap:24px; padding:24px; border-radius:24px; background:linear-gradient(135deg,rgba(167,139,250,.12),rgba(251,114,153,.08)); border:1px solid rgba(167,139,250,.22); }
    ...
    .level-badge { width:44px; height:44px; border-radius:14px; background:linear-gradient(135deg,#a78bfa,#fb7299); display:flex; align-items:center; justify-content:center; flex-shrink:0; box-shadow:0 8px 20px rgba(167,139,250,.35); }
    .level-areas { flex:1; }
    ...
    .step-connector { position:absolute; left:14px; top:32px; bottom:-12px; width:2px; background:linear-gradient(180deg,rgba(167,139,250,.4),rgba(167,139,250,.1)); z-index:0; }
    ...
    .step-active { background:linear-gradient(135deg,#a78bfa,#fb7299); box-shadow:0 4px 12px rgba(167,139,250,.4); }
    .step-pending { background:rgba(167,139,250,.18); border:2px solid rgba(167,139,250,.3); }
    .step-card { flex:1; margin-bottom:12px; }
    .path-prog-track { height:6px; border-radius:99px; background:rgba(167,139,250,.15); overflow:hidden; }
    ...
    .continue-btn { padding:7px 14px; border-radius:10px; background:linear-gradient(135deg,#a78bfa,#fb7299); color:white; font-size:12px; font-weight:600; text-decoration:none; white-space:nowrap; flex-shrink:0; transition:opacity .2s; }
    .continue-btn:hover { opacity:.88; }
    .rec-card { }
    .rec-num { width:28px; height:28px; border-radius:9px; background:linear-gradient(135deg,rgba(167,139,250,.2),rgba(251,114,153,.14)); border:1px solid rgba(167,139,250,.22); display:flex; align-items:center; justify-content:center; font-size:12px; font-weight:800; color:#8b6ef2; flex-shrink:0; }
    .enroll-btn { padding:7px 14px; border-radius:10px; background:rgba(167,139,250,.12); border:1px solid rgba(167,139,250,.22); color:#7c5ce0; font-size:12px; font-weight:600; text-decoration:none; white-space:nowrap; flex-shrink:0; transition:all .22s; }
    .enroll-btn:hover { background:rgba(167,139,250,.22); }
    .tip-card { display:flex; gap:8px; padding:10px 14px; border-radius:13px; background:rgba(167,139,250,.06); border:1px solid rgba(167,139,250,.12); }
  `],
})
