...
@Component({
          ...
          <div class="nav-header">
            ...
            <div class="mt-3">
              <div class="flex items-center justify-between mb-1">
                <span class="text-xs" style="color:#948da3">Progression</span>
                <span class="text-xs font-bold" style="color:#8b6ef2">{{ progress() }}%</span>
              </div>
          ...
          <div class="nav-scroll">
            ...
            <div *ngFor="let mod of modules(); let mi = index" class="module-block">
              <button class="module-header"
                ...
                <div class="flex items-center gap-2 flex-1 min-w-0">
                  <svg *ngIf="moduleStatuses()[mod.id]?.status === 'LOCKED'"
                    width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#948da3" stroke-width="2.5" style="flex-shrink:0">
                ...
                <ng-container [ngSwitch]="moduleStatuses()[mod.id]?.status">
                  ...
                  <span *ngSwitchDefault class="text-xs" style="color:#948da3;white-space:nowrap">{{ completedInModule(mod) }}/{{ mod.lessons.length }}</span>
                </ng-container>
              </button>
              ...
              <div *ngIf="openModules.has(mod.id) && moduleStatuses()[mod.id]?.status !== 'LOCKED'" class="lessons-list">
                ...
                <button *ngIf="moduleQuizMap()[mod.id]" class="lesson-row quiz-module-btn"
                  ...
                  <div class="lesson-icon" style="background:linear-gradient(135deg,#f5a524,#fb7299)">
            ...
            <div *ngIf="!loading() && courseFinalQuiz()" class="final-quiz-nav"
              ...
              <button class="lesson-row final-quiz-btn"
                ...
                <div class="lesson-icon" [style.background]="finalQuizLocked() ? 'rgba(148,141,163,.25)' : 'linear-gradient(135deg,#a78bfa,#f5a524)'">
                  <svg *ngIf="finalQuizLocked()" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#948da3" stroke-width="2.5"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                ...
                <div class="flex flex-col flex-1 min-w-0">
                  <span class="lesson-title" [style.color]="finalQuizLocked() ? '#948da3' : '#7c5ce0'" style="font-weight:700">Quiz Final</span>
            ...
            <div *ngIf="!loading() && modules().length === 0" class="p-6 text-center">
              <p class="text-sm" style="color:#948da3">Aucun module pour ce cours</p>
            </div>
          </div>
        </aside>
        ...
        <main class="content-area">
          ...
          <div *ngIf="!activeLesson() && !activeQuizModuleId() && !activeFinalQuiz() && !loading()" class="flex flex-col items-center justify-center h-full gap-4">
            ...
            <div class="text-center">
              <p class="font-bold font-display" style="color:#221f2c">{{ course()?.title }}</p>
              <p class="text-sm mt-1" style="color:#948da3">Sélectionnez une leçon dans le menu pour commencer</p>
            </div>
          </div>
          ...
          <div *ngIf="activeLesson() && !activeQuizModuleId() && !activeFinalQuiz() && !loading()" class="lesson-content">
            <div class="lesson-topbar">
              <div>
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span *ngIf="activeLesson()!.durationMinutes" class="text-xs" style="color:#948da3">{{ activeLesson()!.durationMinutes }} min</span>
                </div>
            ...
            <div *ngIf="activeLesson()!.type === 'VIDEO' && !activeLesson()!.videoUrl" class="empty-viewer">
              ...
              <p style="color:#948da3">Aucune vidéo disponible</p>
            </div>
            ...
            <div *ngIf="activeLesson()!.type === 'PDF' && !activeLesson()!.pdfUrl" class="empty-viewer">
              ...
              <p style="color:#948da3">Aucun PDF disponible</p>
            </div>
            ...
            <div *ngIf="activeLesson()!.type === 'QUIZ'" class="empty-viewer">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
              <p style="color:#221f2c" class="font-semibold">Quiz lié à cette leçon</p>
            ...
            <div class="lesson-nav-bar">
              ...
              <span class="text-xs" style="color:#948da3">{{ currentLessonIndex() + 1 }} / {{ allLessons().length }}</span>
          ...
          <div *ngIf="(activeQuizModuleId() || activeFinalQuiz()) && !loading()" class="lesson-content">
            <div class="lesson-topbar">
              <div>
                <div class="flex items-center gap-2 mb-1">
                  <span class="type-badge" [style.background]="activeFinalQuiz() ? 'rgba(167,139,250,.14)' : '#f5a52422'"
                    [style.color]="activeFinalQuiz() ? '#7c5ce0' : '#d97706'" style="font-weight:700">
                ...
                <div class="flex items-center gap-3 mt-1">
                  <span class="text-xs" style="color:#948da3">{{ activeModuleQuiz()?.questions?.length }} questions</span>
                  <span class="text-xs" style="color:#948da3">{{ activeModuleQuiz()?.timeLimit }} min</span>
            ...
            <div *ngIf="!quizSubmitted()" class="quiz-questions-area">
              ...
              <div class="quiz-action-bar">
                <span class="text-sm" style="color:#948da3">
            ...
            <div *ngIf="quizSubmitted()" class="quiz-questions-area">
              <div class="score-card" [class.score-card-pass]="quizResult()?.passed">
                ...
                <div>
                  ...
                  <div class="text-sm mt-1" style="color:#948da3">Score minimum : {{ activeModuleQuiz()?.passingScore }}%</div>
                  ...
                  <div *ngIf="quizResult()?.passed && activeFinalQuiz()" class="text-xs mt-2" style="color:#7c5ce0;font-weight:600">
                    ...
                    Félicitations ! Rendez-vous dans les évaluations pour votre badge.
                  </div>
              ...
              <div *ngFor="let q of activeModuleQuiz()?.questions; let qi = index" class="quiz-q-card">
                ...
                <div *ngIf="q.explanation" class="q-expl">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#8b6ef2" stroke-width="2" style="flex-shrink:0"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
                  {{ q.explanation }}
                </div>
              </div>
  ...
  `,
  styles: [`
    .lesson-nav { width:300px; flex-shrink:0; display:flex; flex-direction:column; border-right:1px solid rgba(167,139,250,.14); background:rgba(255,253,251,.92); }
    .nav-header { padding:20px 16px 14px; border-bottom:1px solid rgba(167,139,250,.1); }
    .back-btn { display:inline-flex; align-items:center; gap:5px; font-size:12px; color:#948da3; text-decoration:none; margin-bottom:10px; transition:color .2s; }
    .back-btn:hover { color:#7c5ce0; }
    .course-nav-title { font-size:13px; font-weight:700; color:#221f2c; font-family:'Fraunces',Georgia,serif; line-height:1.35; }
    .prog-track { height:5px; border-radius:99px; background:rgba(167,139,250,.15); overflow:hidden; }
    ...
    .module-header:hover:not(.module-locked), .module-open:not(.module-locked) { background:rgba(167,139,250,.08); }
    .module-locked { cursor:default !important; opacity:.6; }
    .module-locked:hover { background:transparent !important; }
    .mod-title { color:#221f2c; }
    .text-locked { color:#948da3 !important; }
    ...
    .mod-badge-locked { background:rgba(148,141,163,.1); color:#948da3; border:1px solid rgba(148,141,163,.18); }
    ...
    .lesson-row:hover { background:rgba(167,139,250,.07); }
    .lesson-active { background:rgba(167,139,250,.14) !important; border:1px solid rgba(167,139,250,.28); }
    ...
    .lesson-title { font-size:12px; color:#4a4458; font-weight:500; flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
    .quiz-title { color:#d97706 !important; font-weight:600 !important; }
    .quiz-module-btn:hover { background:rgba(245,165,36,.08) !important; }
    /* Quiz Final */
    .final-quiz-nav { padding:8px 4px 4px; border-top:1px solid rgba(167,139,250,.1); margin-top:4px; }
    .final-quiz-locked-nav { opacity:.7; }
    .final-quiz-btn:hover:not(:disabled) { background:rgba(167,139,250,.1) !important; }
    ...
    .lesson-main-title { font-family:'Fraunces',Georgia,serif; font-size:22px; font-weight:700; color:#221f2c; margin:0; }
    ...
    .pdf-player { width:100%; height:520px; border-radius:20px; border:1px solid rgba(167,139,250,.12); display:block; }
    ...
    .lesson-text { font-size:14px; line-height:1.8; color:#4a4458; white-space:pre-wrap; font-family:'Inter',system-ui,sans-serif; }
    .lesson-nav-bar { display:flex; align-items:center; justify-content:space-between; padding:16px 30px; border-top:1px solid rgba(167,139,250,.1); margin-top:auto; }
    .nav-lesson-btn { display:flex; align-items:center; gap:6px; padding:10px 18px; border-radius:14px; background:rgba(167,139,250,.08); border:1px solid rgba(167,139,250,.16); color:#7c5ce0; font-size:13px; font-weight:600; cursor:pointer; font-family:inherit; transition:all .22s; }
    .nav-lesson-btn:hover:not(:disabled) { background:rgba(167,139,250,.18); }
    ...
    .quiz-q-card { background:rgba(255,255,255,.82); border:1px solid rgba(167,139,250,.14); border-radius:18px; padding:20px; }
    .q-header { display:flex; gap:12px; align-items:flex-start; margin-bottom:14px; }
    .q-num { flex-shrink:0; width:28px; height:28px; border-radius:9px; background:linear-gradient(135deg,rgba(167,139,250,.18),rgba(251,114,153,.18)); border:1px solid rgba(167,139,250,.22); display:flex; align-items:center; justify-content:center; font-size:11px; font-weight:700; color:#7c5ce0; }
    .q-text { font-size:14px; color:#221f2c; font-weight:600; line-height:1.5; margin:0; }
    ...
    .q-opt { display:flex; align-items:center; gap:10px; padding:11px 14px; border-radius:12px; background:rgba(167,139,250,.04); border:1px solid rgba(167,139,250,.12); cursor:pointer; font-size:13px; color:#4a4458; font-family:inherit; text-align:left; transition:all .18s; }
    .q-opt:hover { background:rgba(167,139,250,.1); border-color:rgba(167,139,250,.3); }
    .q-opt-selected { background:rgba(139,110,242,.12) !important; border-color:rgba(139,110,242,.45) !important; color:#5c3ec2 !important; font-weight:600; }
    ...
    .opt-letter { flex-shrink:0; width:22px; height:22px; border-radius:7px; background:rgba(167,139,250,.12); display:flex; align-items:center; justify-content:center; font-size:10px; font-weight:700; color:#8b6ef2; }
    .opt-check { border-radius:5px; border:1.5px solid rgba(167,139,250,.3); background:rgba(167,139,250,.06); }
    .q-opt-selected .opt-letter { background:rgba(139,110,242,.2); color:#5c3ec2; }
    ...
    .q-expl { display:flex; align-items:flex-start; gap:7px; margin-top:10px; padding:10px 12px; border-radius:10px; background:rgba(139,110,242,.05); border-left:3px solid #8b6ef2; font-size:12px; color:#6b4ec6; line-height:1.5; }
    .quiz-action-bar { display:flex; align-items:center; justify-content:space-between; padding:16px 0 0; border-top:1px solid rgba(167,139,250,.1); }
    .btn-submit-quiz { display:flex; align-items:center; gap:8px; padding:12px 24px; border-radius:16px; background:linear-gradient(135deg,#a78bfa,#fb7299); border:none; color:white; font-size:14px; font-weight:700; cursor:pointer; font-family:inherit; transition:all .22s; box-shadow:0 4px 16px rgba(167,139,250,.3); }
    .btn-submit-quiz:hover:not(:disabled) { transform:translateY(-1px); box-shadow:0 6px 22px rgba(167,139,250,.42); }
...
export class StudentLearn implements OnInit {
  ...
  progress = computed(() => this.enrollment()?.progress ?? 0);
  ...
  typeColor(type: string): string { return ({ VIDEO: '#a78bfa', PDF: '#fb7299', TEXT: '#1f9d6f', QUIZ: '#f5a524' } as any)[type] ?? '#948da3'; }
